...
public actor AppStateEngine {
    ...
    /// Optional transform applied to every interactive element's
    /// screen-global AX rect (`AXPosition` + `AXSize`) before it's
    /// emitted into the tree markdown. Callers pass `nil` to omit
    /// rects entirely (the cua-mirrored default — `.ax` mode and any
    /// caller that hasn't captured a screenshot yet). `.som` callers
    /// pass a closure that maps screen-global points to
    /// scaled-image pixels (the same space `click(x, y)` accepts), so
    /// the model can read `rect=[x,y,w,h]` straight off the tree and
    /// click `(x + w/2, y + h/2)` without eyeballing the screenshot.
    ///
    /// **Cacty divergence #7.** cua emits no rects in the tree — its
    /// model is expected to derive coordinates from the screenshot
    /// visually. That breaks for Gemini on Google Calendar (and any
    /// SPA where the React widget ignores AXPress so element-index
    /// can't be used): the model guesses pixel coords and the click
    /// misses. See CLAUDE.md § "The background contract — read this
    /// first" for the divergence list.
    public typealias RectTransform = @Sendable (CGRect) -> CGRect

    public func snapshot(
        pid: Int32,
        windowId: UInt32,
        imagePixelTransform: RectTransform? = nil
    ) async throws -> AppStateSnapshot {
        ...
        renderTree(
            ...
            nextIndex: &nextIndex,
            output: &markdown,
            imagePixelTransform: imagePixelTransform
        )
    ...
    private func renderTree(
        ...
        nextIndex: inout Int,
        output: inout String,
        imagePixelTransform: RectTransform?
    ) {
        ...
        // Cacty divergence #7: emit a scaled-image-pixel rect for
        // interactive elements when the caller (SOM mode) supplied a
        // transform. The model reads this rect to drive `click(x, y)`
        // on React/Vue widgets that ignore AXPress. Skipped when the
        // element doesn't report AXPosition+AXSize (top-level
        // containers, AXGroups with no geometry) — the model falls
        // back to either `element_index` clicks or visual estimation.
        if interactive,
           let transform = imagePixelTransform,
           let axRect = attributeRect(element)
        {
            let pxRect = transform(axRect)
            let x = Int(pxRect.origin.x.rounded())
            let y = Int(pxRect.origin.y.rounded())
            let w = Int(pxRect.size.width.rounded())
            let h = Int(pxRect.size.height.rounded())
            line += " rect=[\(x),\(y),\(w),\(h)]"
        }
    ...
    /// AX `AXPosition` + `AXSize` as a CGRect in screen-global points
    /// (top-left origin in AX convention). Returns `nil` when either
    /// attribute is missing — common on containers, AXGroups, and any
    /// element that has no on-screen geometry. Callers that care
    /// about pixel-space rects should remap through a `RectTransform`
    /// supplied to `renderTree` — this helper deliberately returns
    /// the raw AX values so the transform owns the coordinate-space
    /// math.
