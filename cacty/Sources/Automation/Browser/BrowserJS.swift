...
private let log = Logger(subsystem: "com.cacty", category: "browser-js")
...
public enum BrowserJS {
    ...
    ///
    /// **Pre-flight no-op shortcut.** If every Preferences file already
    /// has the flag set, return without quitting. The original cua
    /// implementation always quit-and-relaunched even when the flag
    /// was already true — observed in 2026-05 destroying the user's
    /// entire Chrome session (and the agent's isolated window with
    /// it) when the model called this action defensively after a
    /// `page(execute_javascript)` had already succeeded. The shortcut
    /// makes the action idempotent: a redundant call is free, not
    /// catastrophic.
    public static func enableJavaScriptAppleEvents(bundleId: String) async throws {
        ...
        if isJavaScriptAppleEventsAlreadyEnabled(bundleId: bundleId) {
            log.info("enable_javascript_apple_events: already enabled, skipping quit/patch/relaunch")
            return
        }
    ...
    /// Inspect every Preferences file under the browser's profile
    /// directory and return `true` iff each one already has
    /// `browser.allow_javascript_apple_events == true`. Returns
    /// `false` if any file is missing, unreadable, or has the flag
    /// unset — the safest default, since a false negative just
    /// causes one quit-patch-relaunch cycle, while a false positive
    /// would let a `page` call fail later with a confusing error.
    private static func isJavaScriptAppleEventsAlreadyEnabled(
        bundleId: String
    ) -> Bool {
        guard let profDir = profilesDirectory(for: bundleId) else {
            return false
        ...
        let fm = FileManager.default
        guard let entries = try? fm.contentsOfDirectory(atPath: profDir),
              !entries.isEmpty
        else { return false }

        var sawAtLeastOnePrefs = false
        for entry in entries {
            let path = (profDir as NSString)
                .appendingPathComponent(entry + "/Preferences")
            guard fm.fileExists(atPath: path) else { continue }
            sawAtLeastOnePrefs = true
            guard let data = fm.contents(atPath: path),
                  let json = try? JSONSerialization.jsonObject(with: data)
                    as? [String: Any],
                  let browser = json["browser"] as? [String: Any],
                  let flag = browser["allow_javascript_apple_events"] as? Bool,
                  flag
            else { return false }
        }
        return sawAtLeastOnePrefs
    }
