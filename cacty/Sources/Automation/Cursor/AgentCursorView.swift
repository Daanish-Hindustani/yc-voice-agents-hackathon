...
///
/// **Cacty divergence #6 from cua-driver (color theme only).** Upstream
/// cua paints this overlay in an ice-blue / cyan palette. Cacty repaints
/// it green so the on-screen agent cursor matches the brand and is
/// trivially distinguishable from the user's own cursor. No behavioral
/// change: gradient stops, halo, and focus-rect tints are the only
/// edits. See CLAUDE.md § "Cacty-specific extensions on top of cua".
public struct AgentCursorView: View {
    ...
    private func drawFocusRect(in ctx: GraphicsContext, canvasSize: CGSize) {
        ...
        ctx.fill(
            rounded,
            with: .color(
                Color(red: 0x36/255, green: 0xD3/255, blue: 0x6B/255).opacity(0.08)
            )
        )
        ...
        ctx.stroke(
            rounded,
            with: .color(
                Color(red: 0x36/255, green: 0xD3/255, blue: 0x6B/255).opacity(0.90)
            ),
            lineWidth: 2
        )
        ...
        ctx.stroke(
            rounded,
            with: .color(
                Color(red: 0x36/255, green: 0xD3/255, blue: 0x6B/255).opacity(0.30)
            ),
            lineWidth: 8
        )
    }
    ...
    private func drawCursor(in ctx: GraphicsContext) {
        ...
        // Green gradient fill (Cacty-branded agent-cursor palette).
        ctx.fill(
            transformed,
            with: .linearGradient(
                Gradient(colors: [
                    Color(red: 0xE0/255, green: 0xFB/255, blue: 0xE8/255),
                    Color(red: 0x36/255, green: 0xD3/255, blue: 0x6B/255),
                    Color(red: 0x1A/255, green: 0x82/255, blue: 0x3D/255),
                ]),
        ...
        // Green bloom halo — radial glow that reads as agent presence.
        ...
        ctx.fill(
            ...
            with: .radialGradient(
                Gradient(colors: [
                    Color(red: 0x36/255, green: 0xD3/255, blue: 0x6B/255).opacity(0.45),
                    Color(red: 0x36/255, green: 0xD3/255, blue: 0x6B/255).opacity(0.10),
                    Color(red: 0x36/255, green: 0xD3/255, blue: 0x6B/255).opacity(0.0),
                ]),
                center: p,
                startRadius: 0,
                endRadius: bloomR
