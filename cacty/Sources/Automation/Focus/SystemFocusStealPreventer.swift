...
private final class Dispatcher: @unchecked Sendable {
    private struct Entry {
        ...
        /// **Cacty extension — intentionally diverges from cua.**
        ///
        /// Cached cross-Space verdict captured at `add` time so the
        /// hot demote path doesn't pay `CGWindowListCopyWindowInfo`
        /// (50–200 ms) on every activation. `true` means the
        /// target's primary window is on a different macOS Space
        /// than the user's active one; `false` means same Space;
        /// `nil` means undecided at arm time (cold launch, no
        /// windows yet — treat as same Space and demote, matching
        /// cua's default behavior).
        ///
        /// **Why this divergence is necessary.** cua's restore
        /// primitive is `restoreTo.activate(options: [])`. When
        /// `restoreTo` is in a fullscreen Space and the target is
        /// in a regular Space (or vice versa), `.activate()` asks
        /// macOS to switch Spaces — Mission Control plays its
        /// fade animation. Cacty's voice-PTT UX surface treats
        /// that animation as a critical UX failure ("entire Mac
        /// blinks while agent works"); cua's MCP-CLI users don't
        /// share that constraint. When the target is on a
        /// different Space than the user, we skip the demote
        /// entirely — the target is invisible on its own Space,
        /// so there is nothing to undo. cua does not implement
        /// this skip; it inherits the flash.
        let targetIsOnAnotherSpace: Bool?
    }
    ...
    func add(handle: SuppressionHandle, targetPid: pid_t, restoreTo: NSRunningApplication) {
        // Precompute the Space verdict once at arm time. The target's
        // Space doesn't change during a suppression window — Space
        // migration is gated behind an entitlement we don't hold;
        // see `SpaceMigrator`'s docstring — so caching is sound.
        // Per-activation `CGWindowListCopyWindowInfo` calls would
        // delay the demote enough to leave the target visibly
        // hanging on screen for hundreds of ms.
        let crossSpace: Bool? = {
            switch SpaceMigrator.status(forPid: targetPid) {
            case .onAnotherSpace: return true
            case .onCurrentSpace: return false
        ...
        entries[handle.id] = Entry(
            targetPid: targetPid,
            restoreTo: restoreTo,
            targetIsOnAnotherSpace: crossSpace
        )
    ...
    private func handleActivation(note: Notification) {
        ...
        lock.lock()
        let matched = entries.values.first { $0.targetPid == activatedPid }
        lock.unlock()

        guard let entry = matched else { return }

        // **Cacty extension — Space-aware demote skip.** cua's
        // dispatcher unconditionally calls
        // `restoreTo.activate(options: [])` here. When `restoreTo`
        // and the target live on different Spaces (most painful
        // case: user is in fullscreen Space A, target is on
        // regular Space B), `.activate()` triggers a Mission
        // Control Space-switch animation — the whole screen
        // ripples. For Cacty's voice-PTT UX this is a critical
        // failure; the cached verdict on `Entry` lets us check
        // without paying CGWindowListCopyWindowInfo on every
        // activation. When the target is off-Space the user
        // doesn't see it anyway, so skipping the demote is
        // strictly an improvement. `unknown` falls through to
        // demote — matches the cold-launch case where windows
        // haven't materialized yet (cua's default behavior).
        if entry.targetIsOnAnotherSpace == true {
            return
        }
        let restoreTo = entry.restoreTo
