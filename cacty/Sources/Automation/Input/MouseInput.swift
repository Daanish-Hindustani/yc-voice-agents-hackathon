...
public enum MouseInput {
    ...
    public static func click(
        ...
        //
        // **Cacty extension — `forcePidRoute`.** cua's check is
        // purely `isActive`-based, which yields a wrong routing
        // decision when the target is on another Space than the
        // user: `isActive` is true (because the target IS
        // frontmost on its own Space), but HID-tap delivery lands
        // on whatever app is frontmost on the *user's* visible
        // Space — moving the real cursor and clicking the wrong
        // app. `Engine.click`'s pixel branch sets
        // `forcePidRoute: true` when `SpaceMigrator` reports the
        // target is off-Space; the HID-tap branch is then skipped
        // unconditionally and the pid-routed paths below take over.
        // Documented divergence from cua's `MouseInput.click`.
        let targetIsFrontmost =
            NSRunningApplication(processIdentifier: pid)?.isActive ?? false
        if targetIsFrontmost && !forcePidRoute {
    ...
    public static func drag(
        ...
        // **Cacty extension — `forcePidRoute`.** Same rationale as
        // `MouseInput.click`'s flag: when the target is on another
        // Space, force pid-routed delivery so the gesture lands on
        // the right app without moving the user's real cursor.
        let targetIsFrontmost =
            NSRunningApplication(processIdentifier: pid)?.isActive ?? false
        if targetIsFrontmost && !forcePidRoute {
