...
private let log = Logger(subsystem: "com.cacty", category: "telemetry")
...
public final class TelemetryClient: @unchecked Sendable {
    ...
    public func recordInstallation() {
        ...
        do {
            ...
            if isDebugEnabled() {
                log.error("failed to write install marker: \(error.localizedDescription, privacy: .public)")
            }
        }
    }
    ...
    private func sendEvent(event: String, properties: [String: Any], bypassOptOut: Bool) {
        ...
        if debug {
            log.debug("sending event: \(event, privacy: .public)")
        }
        ...
        Task.detached { [urlSession] in
            do {
                ...
                if debug {
                    ...
                    log.debug("\(event, privacy: .public) status: \(status)")
                }
            } catch {
                if debug {
                    log.error("\(event, privacy: .public) failed: \(error.localizedDescription, privacy: .public)")
                }
            }
        }
    }
    ...
    private func getOrCreateInstallationId() -> String {
        ...
        do {
            ...
            if isDebugEnabled() {
                log.error("failed to persist install id: \(error.localizedDescription, privacy: .public)")
            }
        }
        return newId
    }
