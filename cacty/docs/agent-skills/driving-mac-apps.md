---
name: driving-mac-apps
description: Drive a native macOS app via Cacty's `Automation.Engine` — snapshot the window's AX tree, click/type/scroll by element_index, verify via re-snapshot. Apply when planning any GUI task in a real macOS application (e.g. "open a file in TextEdit", "navigate to /Applications in Finder", "click the Save button in Numbers").
---

# Driving Mac apps

> **Audience.** This is a skill for the agent (Gemini, in Cacty's case)
> to consult when it needs to operate a real macOS application. It
> describes how to think about the tools, not how to invoke them
> mechanically — the actual function calls live in the agent's tool
> schema. The patterns here describe macOS itself: the AX tree, the
> SkyLight pixel-click recipe, focus-steal vectors, Electron/Chromium
> quirks. They apply identically whether the agent is calling
> `Automation.Engine` from inside Cacty or driving the underlying
> engine through some other surface.
>
> **Translation.** Cacty's tool surface mirrors cua-driver's exactly
> as of 2026-05-14. Tool names, argument shapes, and behavior are
> identical: `list_apps`, `list_windows`, `launch_app`,
> `get_window_state`, `click` (element_index or pixel), `type_text`
> (AX `kAXSelectedText`), `type_text_chars` (raw CGEvent), `press_key`
> (bare or with element_index pre-focus), `page` (browser JS / DOM),
> `right_click`, `double_click`, `drag`, `scroll`, `hotkey`,
> `move_cursor`, `zoom`, `get_cursor_position`, `get_screen_size`,
> `get_accessibility_tree`, `screenshot`, `check_permissions`,
> `set_recording`, `get_recording_state`, `replay_trajectory`,
> `get_agent_cursor_state`, `set_agent_cursor_enabled`,
> `set_agent_cursor_motion`, `get_config`, `set_config`.
> For per-tool descriptions and edge cases consult the matching
> `cua-driver/Sources/CuaDriverServer/Tools/<Name>Tool.swift`.

Whenever the user asks to drive a native macOS app, follow the loop
in this skill rather than calling tools ad-hoc — the
snapshot-before-action invariant is not
