...
> **Audience.** Specification of the on-disk **trajectory format**
> produced by `Sources/Automation/Recording/` and consumed by
> Cacty's Phase 1 trajectory replay viewer. Each task can optionally
> emit a `turn-NNNNN/{app_state.json, screenshot.png, action.json,
> click.png}` directory tree, suitable for demos, regression diffs,
> and training-data export. **Read the format spec; ignore the
> CLI/MCP command examples** (`cua-driver recording start ...`,
> `set_recording`, `cua-driver serve &`) — those describe an upstream
> wrapper that Cacty doesn't ship; recording is controlled by direct
> Swift calls into `RecordingSession`.
