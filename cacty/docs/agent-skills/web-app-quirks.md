...
> **Audience.** Skill for the agent to consult when the target app
> renders its UI in a web runtime — Chromium (Chrome, Edge, Brave,
> Arc), WebKit (Safari, Mail), Electron (Slack, Discord, VS Code,
> Figma), Tauri. The quirks below describe how the underlying
> renderer reacts to AX queries and synthetic input — they describe
> macOS itself, not the surface that exposes the tools. Argument
> shapes (`pid`, `window_id`, `element_index`) match identically
> across Cacty's `Automation.Engine` and any other engine wrapper.
